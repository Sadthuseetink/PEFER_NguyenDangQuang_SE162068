...
const FormAddEdit = () => {
    ...
    const updateStaff = async (staffID, data) => {
        ...
        if (res.status === 200) {
            toast.success(`Update Staff with ID: ${staffID} successfully <3333`);
            navigate('/dashboard/');
        }
    }
    ...
    const addNewStaff = async (data) => {
        ...
        if (res.status === 200 || res.status === 201) {
            toast.success("New Staff has been added successfully <333");
            navigate('/dashboard/');
        }
    }
    ...
    const validateForm = () => {
        ...
        if (name.trim() === '' || name.length < 2) {
            ...
            if (name.length < 2) {
                errors.name_err = 'Name must more than 2 words';
            }
            isvalid = false;
        }
        ...
        if (address.trim() === '') {
            errors.address_err = 'Add ress is Required';
            isvalid = false;
        }
    ...
    return (
        <div className="container">
            <div className="form">
                <h>{id ? "Form Update" : "Add New Staff"}</h>
                <form onSubmit={handleSubmit}>
                    ...
                    {id && (
                        <div>
                            ...
                            <input type="text" name='createAt' value={new Date(state.createdAt * 1000).toLocaleDateString()} readOnly />
                        </div>
                    )}
